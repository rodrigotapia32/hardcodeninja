...
function showLoginUI(onSubmit) {
    ...
    document.getElementById("startBtn").onclick = () => {
        const nick =
            document.getElementById("nickInput").value.trim() || "Ninja";
    ...
    showLoginUI(({ nick, color }) => {
        ...
        socket.on("connect", () => {
            ...
            window.onPhaserReady = (player) => {
                ...
                console.log("[CLIENT] Emit newPlayer:", {
                    x: player.x,
                    y: player.y,
                    name: nick,
                    color,
        ...
        socket.on("ability", (data) => {
            ...
            if (!window.updateRemotePlayers) {
                console.log(
                    "[CLIENT] updateRemotePlayers no está definido, return",
                );
                return;
            }
            const id = data.from;
            ...
            function tryAbility() {
                ...
                console.log(
                    "[CLIENT] Buscando remotePlayer para id",
                    id,
                    "Intento",
                    tries,
                    "remotePlayers:",
                ...
                if (!remote) {
                    ...
                    if (tries < 5) {
                        ...
                        console.log(
                            "[CLIENT] No se encontró remotePlayer para id",
                            id,
                ...
                if (data.type === "deflect") {
                    console.log(
                        "[CLIENT] Ejecutando activateDeflect en remotePlayer",
                        id,
                    );
                    remote.activateDeflect();
                }
                if (data.type === "blink")
                    remote.tryBlink(data.x, data.y, remote.scene.walls, 180);
                if (data.type === "shockwave") {
                    const allPlayers = [
                        window.player,
                        ...Object.values(window.remotePlayers || {}),
                    ];
                ...
                if (data.type === "dagger") {
                    ...
                    if (
                        !target &&
                        window.player &&
                        window.player.id === data.targetId
                    ) {
                        target = window.player;
                        console.log(
                            "[CLIENT] Dagger: target es el jugador local",
                            window.player.nick,
                        );
                    }
                    console.log(
                        "[CLIENT] Ejecutando launchDagger en remotePlayer",
                        id,
                        "contra target",
                        data.targetId,
                        target,
                    ...
                    if (target) {
                        ...
                        console.log(
                            "[CLIENT] Dagger: target no encontrado para id",
                            data.targetId,
                        );
                    }
                }
        ...
        socket.on("victory", (data) => {
            alert(
                `¡Victoria! ${data.winner} ha ganado con ${data.score} puntos. La partida se reiniciará en 3 segundos.`,
            );
        });
        window.sendMove = (x, y) => {
