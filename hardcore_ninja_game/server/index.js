...
io.on("connection", (socket) => {
    ...
    socket.on("newPlayer", (data) => {
        ...
        console.log(
            "[SERVER] Estado actual de players tras newPlayer:",
    ...
    socket.on("move", (data) => {
        ...
        console.log(
            "[SERVER] Estado actual de players tras move:",
            JSON.stringify(players),
        );
        io.emit("state", players);
    });
    ...
    socket.on("updateState", (data) => {
        ...
        if (data.killerId && data.alive === false && players[data.killerId]) {
            players[data.killerId].score =
                (players[data.killerId].score || 0) + 1;
            // Si llega a 5 puntos, notificar victoria
            if (players[data.killerId].score >= 5) {
                io.emit("victory", {
                    winner: players[data.killerId].name,
        ...
        console.log(
            "[SERVER] updateState recibido:",
            data,
            "socket.id:",
            socket.id,
        );
        console.log(
            "[SERVER] Estado actual de players tras updateState:",
            JSON.stringify(players),
        );
        io.emit("state", players);
    });
    ...
    socket.on("disconnect", () => {
        ...
        console.log(
            "[SERVER] Estado actual de players tras disconnect:",
            JSON.stringify(players),
        );
        io.emit("state", players);
    });
